...
def build_delta_m_matrix(delta, indices):
    """Construit la matrice delta réduite aux liens et chemins multiples."""
...
def build_delta_m_delta_mu_delta_um(delta, delta_m, gamma_m, indices, network, path_list):
    """Construit les matrices delta réduites et élimine les lignes linéairement dépendantes."""
    n = len(network.sn)
    p = sum(len(k) for k in path_list)
    n_m = indices["n_m"]
    m_m = indices["m_m"]
...
def build_B_q0(delta_m, delta_m_tilde, delta_m_u, network, final_indices):
    """Construit la matrice B et le vecteur q0."""
    m = len(network.q_od)
    q_od = network.q_od
...
def extract_dimensions(delta_m, delta_m_tilde, A, T_m):
    ...
    if u1 + s1 != r1:
        raise ValueError(f"Erreur : u1 + s1 ≠ r1 (u1={u1}, r1={r1}, s1={s1})")
...
def lam_solver_linear_system(network, final_indices, dimensions, alpha, beta, eps_num, 
    ...
    """Résout le système linéarisé via système linéaire."""
...
def lam_solver_qp(network, final_indices, dimensions, alpha, beta, eps_num, 
    ...
    """Résout le système linéarisé via problème quadratique."""
    t0, C = network.t0, network.C
    t0_lin, K = cf.linearised_bpr_matrices(t0, C, alpha, beta, eps_num)
    n1, r1, s1, u1 = dimensions["n1"], dimensions["r1"], dimensions["s1"], dimensions["u1"]
    links_m = final_indices["links_m"]
...
def compute_lam_solution(network, path_list, G, msa_flows, method='qp', alpha=0.15, beta=4):
    """
    ...
    Args:
        ...
        G: Graphe NetworkX
        msa_flows: Flux numériques MSA (pour calculer eps)
    ...
    # 5. Calcul de eps (niveau de congestion)
    eps_num = msa_flows / network.C
    
    # 6. Résolution selon la méthode choisie
    if method == 'qp':
